...
export const BtnMenuTypeProduct = ({ dataTestId, categories, setIsActivePage, sortedProducts, onCloseBurger }) => {
    ...
    return (
        <li className='container-btn-menu'>
            ...
            >
                <span 
                    data-test-id={`${dataTestId}-${categories.path === 'all' ? 'books' : categories.path}`}
                >
                    { categories.name }
                </span>

                <span 
                    className='quantity-type-product'
                    data-test-id={`${dataTestId}-book-count-for-${categories.path}`}
                >{`${categories.id !== 0 ? sortedProducts[categories.path]?.length || 0 : ''}`}</span>
            </NavLink>
        </li>
    )
